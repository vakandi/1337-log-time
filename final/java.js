// java.js
const ruby = RubyJS.createContext();
...
# Assuming the API output is stored in a variable called `api_output`       api_output_json = JSON.parse(response.body)

daily_hours = {}
                                                                            api_output_json.each do |date, time_string|
    # Parse the time string into hours, minutes, and seconds                    hours, minutes, seconds = time_string.split(':').map(&:to_i)
...
daily_hours.each do |date, hours|
    total_hours += hours
    minutes = (hours - hours.to_i) * 60                                        # puts "#{date}: #{hours.to_i} hours and #{minutes.to_i} minutes"
end                                                                         puts "Total hours: #{total_hours.to_i} h"
`;
...
submitForm.addEventListener("submit", (event) => {
    ...
    try {
        const result = ruby.run(rubyCode, loginValue);
        console.log(result);
        // display the result on the page here
    } catch(error) {
        console.error(error);
    }
});
